...
import { redis, CACHE_KEYS } from '../src/lib/redis.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    body = SubmitCheckSchema.parse(req.body);
  } catch (e) {
  ...
  if (userLat !== undefined && userLng !== undefined) {
    const { data: geofenceResult } = await supabase.rpc('is_point_in_terminal_geofence', {
  ...
  const trustWeight = Math.min(3.0, reputationScore * (isGeofenced ? 2.5 : 1.0));
